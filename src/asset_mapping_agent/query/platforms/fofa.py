...
class FofaCompiler(BaseQueryCompiler):
    def __init__(self) -> None:
        super().__init__(PLATFORM_CAPABILITIES["fofa"])
    ...
    def render_predicate(self, predicate: Predicate) -> str:
        ...
        if operator is ComparisonOperator.IN:
            values = predicate.value if isinstance(predicate.value, (list, tuple, set)) else [predicate.value]
            return "(" + " || ".join(f"{field}={self._quote(value)}" for value in values) + ")"
        if operator is ComparisonOperator.EXISTS:
            return f'{field}!=""'
        raise ValueError(f"Unsupported operator for FOFA compiler: {operator.value}")
