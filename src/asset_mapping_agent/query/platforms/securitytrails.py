...
class SecurityTrailsCompiler(BaseQueryCompiler):
    ...
    def __init__(self) -> None:
        super().__init__(PLATFORM_CAPABILITIES["securitytrails"])
    ...
    def compile(self, intent: QueryIntent) -> CompiledQuery:
        ...
        if hostname:
            filters_used.append(f"hostname eq {hostname}")
        else:
            warnings.append("SecurityTrails current compiler requires a domain or hostname target.")

        if len(unique_candidates) > 1:
            warnings.append(
                "SecurityTrails current adapter executes a single hostname lookup; extra hostname candidates were moved to post-filter."
            )
            post_filters.extend(f"hostname eq {candidate}" for candidate in unique_candidates[1:])

        if intent.options.aggregate:
            warnings.append("SecurityTrails current adapter ignores aggregate mode.")
        ...
        return CompiledQuery(
            ...
            metadata={
                ...
                "lookup_mode": intent.options.native_flags.get("securitytrails_mode", "domain"),
            },
        )

    def render_predicate(self, predicate: Predicate) -> str:
        raise NotImplementedError("SecurityTrails compiler uses custom domain extraction instead of predicate rendering.")
